...
import {Button, Checkbox, Form, Input, message, Popconfirm, Space, Upload} from "antd";
...
import '../attr.css'
import {columnsPart2, generateData, TableItem, transformSkuListToForm} from "../ProductCommon";
...
function AttrUpdateForm(props: {
    ...
    const [columns, setColumns] = useState<Array<TableItem>|undefined>(()=>{
        const attrForm =  transformSkuListToForm(initAttrValues);
        if (Object.keys(attrForm).length === 0)
            return undefined;
        const columnsPart1 = Object.keys(attrForm).map(key => ({title: key, dataIndex: key}))
        return columnsPart1.concat(columnsPart2);
    });
